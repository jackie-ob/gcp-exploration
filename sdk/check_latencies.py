...
NUM_CYCLES_PER_WORKER = 10

def upload_download_delete_cycle():
    t = time.time()
...
def do_it(num_generations, multiplier, use_processes):
    ...
    for gen in range(num_generations):
        ...
        for i in range(parallelism):
            f = pool.submit(upload_download_delete_cycle)
            futures.append(f)
        ...
        print("Gen %d (x %d): mean=%.3f stdev=%.3f mean_per_operation=%.3f" % (gen, parallelism, mean, stdev, mean_per_operation))
...
def main():
    ...
    do_it(args.num_generations, args.multiplier, args.use_processes)
    return 0
